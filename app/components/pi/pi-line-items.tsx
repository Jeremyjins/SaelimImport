...
interface PILineItemsProps {
  products: Product[];
  initialItems?: PILineItem[];
}

// 내부 상태용 타입 — React key에 _rowId 사용
type LineItemRow = PILineItem & { _rowId: string };
...
function withRowId(item: PILineItem): LineItemRow {
  return { ...item, _rowId: crypto.randomUUID() };
}

export function PILineItems({ products, initialItems }: PILineItemsProps) {
  ...
  function updateItem(index: number, patch: Partial<PILineItem>) {
    setItems((prev) => {
      const next = [...prev];
  ...
  // 직렬화 시 _rowId 제외
  const serialized: PILineItem[] = items.map(
