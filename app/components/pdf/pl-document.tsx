...
// PL: no Unit Price / Amount columns — weight-focused
const COLUMNS: PDFColumn[] = [
  { key: "no", label: "No.", width: "5%", align: "center" },
  { key: "product", label: "Product", width: "38%", align: "left" },
  { key: "gsm", label: "GSM", width: "10%", align: "center" },
  { key: "width", label: "Width (mm)", width: "12%", align: "center" },
  { key: "qty", label: "Qty (KG)", width: "35%", align: "right" },
];

export function PLDocument({ data: shipping }: PLDocumentProps) {
  ...
  const totalQty = shipping.details.reduce(
    (sum, item) => sum + item.quantity_kg,
    0
  );
  ...
  const totalRow: Record<string, string> = {
    ...
    gsm: "",
    width: "TOTAL",
    qty: formatPdfNumber(totalQty, 0) + " KG",
  };
  ...
  return (
    <Document>
      <Page size="A4" style={styles.page}>
        <PDFHeader
          title="PACKING LIST"
          docNo={shipping.pl_no}
          date={formatPdfDate(shipping.ci_date)}
          subtitle={`Ref CI: ${shipping.ci_no}`}
        />
        <PDFParties
          left={{
            label: "SHIPPER",
            ...
            <View style={{ flexDirection: "row" }}>
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", marginBottom: 3 }}>
                  <Text style={{ fontSize: 7, color: COLORS.label, width: "35%" }}>
                    Vessel:
                  </Text>
              ...
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: "row", marginBottom: 3 }}>
                  <Text style={{ fontSize: 7, color: COLORS.label, width: "25%" }}>
                    ETD:
                  </Text>
                  <Text style={{ fontSize: 8, color: COLORS.text }}>
        ...
        >
          <View style={{ width: "40%", alignItems: "center" }}>
            ...
            >
              <Text style={{ fontSize: 8, color: COLORS.label }}>
