...
interface ShippingLineItemsProps {
  products: Product[];
  initialItems?: ShippingLineItem[];
}

// 내부 상태용 타입 — React key에 _rowId 사용
type LineItemRow = ShippingLineItem & { _rowId: string };
...
function withRowId(item: ShippingLineItem): LineItemRow {
  return { ...item, _rowId: crypto.randomUUID() };
}

export function ShippingLineItems({ products, initialItems }: ShippingLineItemsProps) {
  ...
  function updateItem(index: number, patch: Partial<ShippingLineItem>) {
    setItems((prev) => {
      const next = [...prev];
  ...
  // 직렬화 시 _rowId 제외
  const serialized: ShippingLineItem[] = items.map(
        ...
        <div key={item._rowId}>
          ...
          <div className="md:hidden border rounded-lg p-3 flex flex-col gap-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <label className="text-xs text-zinc-500 mb-1 block">판매단가</label>
                <Input
                  type="number"
                  min={0}
                  step={0.0001}
